...
public class RDSJanitorResourceTracker implements JanitorResourceTracker {
	...
	private void insertResource(Resource resource, String json) {
		...
		LOGGER.debug(String.format("Insert statement is '%s'", insertStatement));
		int updated = this.jdbcTemplate.update(insertStatement,
				resource.getId(),
	...
	private String createInsertStatement() {
		return String.format("insert into %s (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,additionalFields) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", table,
	...
	private void updateResource(Resource resource, String json) {
		...
		LOGGER.debug(String.format("Update statement is '%s'", updateStatement));
		int updated = this.jdbcTemplate.update(updateStatement, resource.getResourceType().toString(), value(resource.getRegion()), emailValue(resource.getOwnerEmail()),
	...
	private String createUpdateStatement() {
		return String.format("update %s set %s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=?,%s=? where %s=? and %s=?", table,
                    AWSResource.FIELD_RESOURCE_TYPE, AWSResource.FIELD_REGION, AWSResource.FIELD_OWNER_EMAIL, AWSResource.FIELD_DESCRIPTION, AWSResource.FIELD_STATE,
                    AWSResource.FIELD_TERMINATION_REASON, AWSResource.FIELD_EXPECTED_TERMINATION_TIME, AWSResource.FIELD_ACTUAL_TERMINATION_TIME, AWSResource.FIELD_NOTIFICATION_TIME,
                    AWSResource.FIELD_LAUNCH_TIME, AWSResource.FIELD_MARK_TIME, AWSResource.FIELD_OPT_OUT_OF_JANITOR, "additionalFields", AWSResource.FIELD_RESOURCE_ID, AWSResource.FIELD_REGION);
	}
